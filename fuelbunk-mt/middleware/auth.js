...
function authorize(...roles) {
  return (req, res, next) => {
    ...
    // Super admin tokens have no stationId — reject them on station-level routes
    if (req.user.isSuperAdmin) return res.status(403).json({ success: false, error: 'Super admin must use /api/super routes.' });
